...
from search_utils import (basis_first_search, pick_strategy, smallest_wl_block_first_search, selective_wl2_split,
                          )
...
def find_graph_automorphism(pauli_sum: PauliSum,
    ...
    return smallest_wl_block_first_search(
        pauli_sum,
        independent_labels=[i for i, v in enumerate(basis_mask) if v],
        dependent_labels=[i for i, v in enumerate(basis_mask) if not v],
        S_mod=S, p=p,
...
if __name__ == "__main__":
    ...
    for _ in range(100):
        ...
        perms = find_graph_automorphism(H, k_wanted=1, ir_rounds=2,
                                        ...
                                        include_parallel=False, include_fundamental_circuits=False,
        ...
        if len(perms) == 0:
            print('failed automorphism', H.phases, H.weights)
            # print(H)
            failed += 1
        # else:

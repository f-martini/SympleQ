...
from .modular_helpers import modp, rank_mod, _solve_linear, nullspace_mod
...
def poly_divmod(a: np.ndarray, b: np.ndarray, p: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    a = modp(poly_trim(a.copy()), p)
    b = modp(poly_trim(b.copy()), p)
    ...
    while len(r) >= len(b) and not (len(r) == 1 and r[0] == 0):
        k = len(r)-len(b)
        c = modp(r[-1] * inv_lead, p)
        q[k] = c
        r[k:k+len(b)] -= c * b
        r = modp(poly_trim(r), p)
        if len(r) == 0:
            r = np.zeros(1, dtype=np.int64)
            break
        if len(r) < len(b):
            break
    return modp(poly_trim(q), p), modp(poly_trim(r), p)
...
def poly_gcd(a: np.ndarray, b: np.ndarray, p: int) -> np.ndarray:
    a = modp(poly_trim(a), p); b = modp(poly_trim(b), p)
    while not (len(b) == 1 and b[0] == 0):
        _, r = poly_divmod(a, b, p)
        a, b = b, r
    return poly_monic(a, p)

def poly_powmod(a: np.ndarray, e: int, m: np.ndarray, p: int) -> np.ndarray:
...
def poly_reciprocal(poly: np.ndarray, p: int) -> np.ndarray:
    poly = poly_trim(poly)
    d = len(poly) - 1
    # q*(t) = t^d q(1/t) => reverse coeffs; then make monic
    rev = poly[::-1].copy()
...
def squarefree_decomposition(f: np.ndarray, p: int) -> List[Tuple[np.ndarray, int]]:
    ...
    if len(g) != 1 or g[0] != 1:
        ...
        groot = poly_pth_root(g, p)
        sub = squarefree_decomposition(groot, p)
...
def equal_degree_factorization(f: np.ndarray, d: int, p: int, rng=np.random.default_rng(2025)) -> List[np.ndarray]:
    ...
    while True:
        ...
        a = modp(rng.integers(0, p, size=len(f)-1, dtype=np.int64), p)
        a = poly_trim(a)
        if len(a) == 0:
            a = np.array([1], dtype=np.int64)
        # compute g = gcd(a^{(p^d - 1)/2} - 1, f)
        exp = (p**d - 1) // 2
        apow = poly_powmod(a, exp, f, p)
        g = poly_gcd(poly_sub(apow, np.array([1], dtype=np.int64), p), f, p)
        if len(g) > 1 and not np.array_equal(g, f):
            return equal_degree_factorization(g, d, p, rng) + equal_degree_factorization(poly_monic(poly_divmod(f, g, p)[0], p), d, p, rng)
...
def minimal_poly_for_vector(F: np.ndarray, v: np.ndarray, p: int) -> np.ndarray:
    ...
    while True:
        ...
        if r2 == r:
            ...
            c = _solve_linear(modp(V, p), modp(-w, p), p)  # particular
...
def primary_components_from_cyclic(F: np.ndarray, p: int) -> Dict:
    ...
    for key in keys:
        ...
        qstar = poly_reciprocal(q, p)
        kstar = tuple(qstar.tolist())
        primaries[key]["reciprocal_key"] = kstar
        primaries[key]["self_reciprocal"] = (kstar == key)
    ...
    for key in primaries.keys():
        ...
        if data["self_reciprocal"]:
            ...
            kstar = data["reciprocal_key"]
            if kstar not in primaries:
                ...
                W = np.concatenate([data["V_basis"], primaries[kstar]["V_basis"]], axis=1)
                half_floor = data["deg"] * data["exponent"]
                sectors.append({
                    "type": "paired",
                    "q_key": key,
                ...
                used.add(key); used.add(kstar)

    Lmin_star = max([sec["half_dim_floor"] for sec in sectors] + [0])
...
def rcf_prepass(F: np.ndarray, p: int) -> Dict:
    ...
    half-dimension floor Lmin* for the largest block.
    """
